...
as.list(c2)
```
If we use complete as our linkage method and assume that there are 2 clusters, then we see one large cluster and one small one. 
...
posterior$component <- ifelse(posterior$comp.1 > 0.3, 1, 2) #maually, say prob > 30% -> cluster 1
